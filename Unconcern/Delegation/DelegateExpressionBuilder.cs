...
namespace Unconcern.Delegation
{
    public class DelegateExpressionBuilder : IDelegateExpressionBuilder
    {
        ...
        public IDelegateExpressionBuilder Do<T>(Func<EventBus.Message<T>, Task> handle)
        {
            return new DelegateExpressionBuilder(Expression.WithHandler(async msg => {
                if (!msg.Type.IsAssignableTo(typeof(T)))
                    throw new InvalidCastException("wrong_handler");
                var content = (T)msg.Content;
                await handle(new EventBus.Message<T>(msg.Timestamp, content, msg.Sender, msg.Recipients));
            }));
        }

        public IDelegateExpressionBuilder Send<T, U>(Func<EventBus.Message<T>, Task<EventBus.Message<U>>> transform)
        {
            return new DelegateExpressionBuilder(Expression.WithReply(async msg => {
                if (!msg.Type.IsAssignableTo(typeof(T)))
                    throw new InvalidCastException("wrong_reply");
                var content = (T)msg.Content;
                var transformed = await transform(new EventBus.Message<T>(msg.Timestamp, content, msg.Sender, msg.Recipients));
                return new EventBus.Message(DateTime.Now, typeof(U), transformed.Content, transformed.Sender, transformed.Recipients);
            }));
        }

        public IDelegateExpressionBuilder When<T>(Func<EventBus.Message<T>, Task<bool>> cond)
        {
